...
export function CopilotLauncher({ configured, label }: { configured: boolean; label: string }) {
  return (
    <section aria-label="Copilot" className="mt-8 grid gap-5 rounded-panel border border-accent/50 bg-surface p-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,2fr)]">
      <div className="flex gap-4">
        <span aria-hidden="true" className="flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-accent/10">
          <svg width="26" height="26" viewBox="0 0 24 24" fill="var(--accent)"><path d="M12 2l1.9 6.1L20 10l-6.1 1.9L12 18l-1.9-6.1L4 10l6.1-1.9z" /><path d="M19 15l.8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8z" opacity="0.7" /></svg>
        </span>
        <div>
          ...
          <button type="button" onClick={() => openCopilot()} disabled={!configured} className="mt-3 rounded-control bg-accent px-4 py-2 text-15 font-medium text-white disabled:opacity-60">
      ...
      <div>
        ...
        <ul className="mt-2 grid gap-2 sm:grid-cols-2">
          {EXAMPLES.map((q) => (
            <li key={q}>
              <button type="button" onClick={() => openCopilot(q)} disabled={!configured} className="w-full rounded-control border border-line bg-bg px-3 py-2 text-left text-15 hover:border-accent disabled:opacity-50">
                {q}
              </button>
            </li>
          ))}
