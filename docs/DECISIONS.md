...
## M10 — BreakoutLabs' own theme

### The product wears the company's colours and typeface
- What: Assistant (the typeface breakoutlab.co sets for body and headings) replaces IBM Plex Sans and Fraunces; the palette is the logo's dark teal (`--brand`, #03444c), their lime highlight (`--lime`, #d0f579), an off-white page and white surfaces. Light is the default because their site is light; the dark scheme keeps the same teal as the paper and lets the lime carry the brand. The rail is always the logo's teal with the wordmark cropped out of the square PNG. The stuck and danger colour is their coral, darkened for text contrast; it is no longer the primary-action colour, which is the brand teal (`bg-brand text-on-brand`) so a Confirm button and a stuck kit stop sharing a colour.
- Why: Steven asked for the app to look like the company it is built for, with the logo in the loop. §14's petrol and garnet were a placeholder identity; this is theirs. Research was the site's own HTML (fonts, colour scheme variables, logo file), because the sandbox's proxy could not render the page in a browser.
- Charts: the three revenue series and the six segment hues were re-validated with the dataviz palette script on the new surfaces (white and #03444c). Light passes; dark passes with contrast warnings that the direct labels and tables satisfy. The live and seeded badge inks were darkened to clear 4.5:1 on the off-white page.
- Landing: the loop's labels are bold with a page-coloured halo so they read across the ring, anchored outside it so "Blueprint" and "Data" no longer sit on the arc, and the wordmark fills the middle. The Enter button is lime.
