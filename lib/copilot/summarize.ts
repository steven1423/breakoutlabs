...
type SummaryClient = { messages: { create: (params: Anthropic.MessageCreateParamsNonStreaming) => Promise<Anthropic.Message> } };
...
export async function summarizeTicket(db: ServiceClient, client: SummaryClient, model: string, ticketId: string): Promise<AiSummary> {
  ...
  const summary = await askModel(client, model, JSON.stringify(context));
...
async function askModel(client: SummaryClient, model: string, ticketJson: string): Promise<AiSummary> {
  let lastError = "";
